...
import { CHECK_AUTH, LOGIN, LOGOUT, REGISTER, SHOW_SNACKBAR } from './actionsTypes'
...
export const loginUser =
	(username: string, password: string): ThunkResult<void> =>
	(dispatch: Dispatch) => {
		const storedUsers = JSON.parse(localStorage.getItem('users') || '[]')
		const user = storedUsers.find(
			(usr: { username: string; password: string }) => usr.username === username && usr.password === password,
		)
